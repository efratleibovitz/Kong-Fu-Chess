...
class TestSession:
    def test_login_with_session_returns_user_id_and_token(self):
        with patch("server.auth.login", return_value=42), \
             patch("server.auth.create_session", return_value="valid_token"):
    ...
    def test_get_user_id_by_token_returns_correct_user_id(self):
        with patch("server.auth._db_get_token", return_value=42):
    ...
    def test_get_user_id_by_token_returns_none_for_forged_token(self):
        with patch("server.auth._db_get_token", return_value=None):
            from server.auth import get_user_id_by_token
